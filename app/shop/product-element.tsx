...
export default function ProductElement({id, name, description, active, stock, image0, price, deleteButton}: {
  id: string,
  name: string,
  description: string,
  active: boolean,
  ...
  function DecreaseAmount(Event?: MouseEvent<HTMLButtonElement>) {
    if (Event?.currentTarget) {
      ...
      InfoRef.current!.classList.remove("text-red-600", "text-green-600");
      if (Amount > 1) {
        SetAmount(Amount - 1);
      } else {
  ...
  function IncreaseAmount(Event?: MouseEvent<HTMLButtonElement>) {
    if (Event?.currentTarget) {
      ...
      const TrueAmount = (!deleteButton ? Amount + (Cart[id] || 0) : Amount);
  ...
  if (active) {
    if (!deleteButton) {
      ...
      return (
        <section className="flex flex-col flex-wrap md:flex-row md:flex-nowrap justify-center items-center gap-x-12">
          ...
          <main className="flex flex-col flex-wrap gap-y-2">
            <Link href={`/shop/products/${id.replace("prod_", "")}`}>
              <h2 className="text-4xl">{name}</h2>
            </Link>
            <p>{description}</p>
            <p>{`$${(price * Amount).toString()}`}</p>
            ...
            <Button onClick={function(Event) {
              ...
              for (let Index = 0; Index < Product.children.length; Index++) {
                Product.children.item(Index)!.classList.add("hidden");
              };            
              InfoRef.current!.innerText = "Item successfully removed from cart!";
              InfoRef.current!.classList.add("text-green-600");
              InfoRef.current!.classList.remove("hidden");
    ...
    if (deleteButton) {
      ...
      delete Cart[id];
      CookieStore.set("Cart", JSON.stringify(Cart));
      return <p>{`This product (id: ${id}) no longer exists and therefore has been deleted from your cart. If you think this was a mistake, please contact support and include the id of the product. Thank you!`}</p>
    } else {
      return (
