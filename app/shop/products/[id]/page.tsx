      ...
      <main>
        ...
        <main className="p-8">
          <Suspense fallback={
            ...
            {
              Product ? <ProductElement cookies={cookies().toString()} product={JSON.stringify(Product)} price={JSON.stringify(await api.stripeRouter.getPriceInformation.query({ priceId: Product.default_price as string }) || "{}")} button="add" /> : <main className="flex flex-col flex-wrap justify-center items-center before:absolute before:inset-0 before:bg-cover before:bg-no-repeat before:bg-center before:h-full before:w-full before:-z-[1] before:shop-bg-image before:brightness-50 text-center p-4 min-[300px]:p-12 md:p-24">
