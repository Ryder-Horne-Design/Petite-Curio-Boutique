...
export function ExpandImage(Event: MouseEvent<HTMLButtonElement>, Source: string) {
  if (Event?.currentTarget) {
    const Button = Event.currentTarget;
    const ImageRef = createRef<HTMLImageElement>();  
    if (Button.childElementCount === 1) {
      const ButtonImage = Button.children[0] as HTMLImageElement;
      const Expanded = document.createElement("div");
      Expanded.className = "z-50 fixed top-0 w-full h-full bg-black/30 flex flex-col justify-center items-center transition-opacity duration-300 opacity-0";
      ...
      ExpandedRoot.render(
        <>
          ...
          <div role="status">
            <svg aria-hidden="true" className="w-8 h-8 text-slate-200 animate-spin fill-yellow-500" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <p className="text-white text-xl text-center w-3/4">Loading...</p>
        </>
      );
        ...
        try {
          await new Promise(function(Resolve, Reject) {
            if (ImageRef.current?.complete) {
              Resolve(ImageRef.current.complete);
            };
          ...
          ExpandedRoot.render(
            <>
              ...
              <p className="text-white text-xl text-center w-3/4">{ButtonImage.alt}</p>
            </>
          );
        } catch(Err) {
          console.warn(Err);
          ExpandedRoot.render(
            <>
              ...
              <p className="text-white text-xl text-center w-3/4">Error Loading Image</p>
            </>
          );
        };
      });
